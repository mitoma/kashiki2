    ...
    pub fn parse_setting(setting_string: String) -> Vec<KeyBind> {
        ...
        for line in setting_string.lines() {
            ...
            let command = settings
                .pop()
                .and_then(|command| Self::parse_action(String::from(command)));
            let strokes: Vec<KeyWithModifier> = settings
                .iter()
                .flat_map(|s| Self::parse_keywithmodifier(String::from(*s)))
                .collect();
...
mod tests {
    ...
    fn parse_setting() {
        assert_eq!(
            ActionStoreParser::parse_setting(
                r"
                #hello
                Return system:enter
                C-X C-S system:save
    ...
    fn parse_keywithmodifier_none() {
        assert_eq!(
            ActionStoreParser::parse_keywithmodifier("".to_owned()),
            None
        );
        assert_eq!(
            ActionStoreParser::parse_keywithmodifier("     ".to_owned()),
        ...
        assert_eq!(
            ActionStoreParser::parse_keywithmodifier("# is comment line".to_owned()),
            None
        );
    }
    ...
    fn parse_keywithmodifier_ok() {
        assert_eq!(
            ActionStoreParser::parse_keywithmodifier("C-A-S-Return".to_owned()).unwrap(),
            KeyWithModifier::new(keys::KeyCode::Return, keys::ModifiersState::CtrlAltShift)
        );
        assert_eq!(
            ActionStoreParser::parse_keywithmodifier("Return".to_owned()).unwrap(),
            KeyWithModifier::new(keys::KeyCode::Return, keys::ModifiersState::NONE)
        );
        assert_eq!(
            ActionStoreParser::parse_keywithmodifier("S-C".to_owned()).unwrap(),
            KeyWithModifier::new(keys::KeyCode::C, keys::ModifiersState::Shift)
        );
        assert_eq!(
            ActionStoreParser::parse_keywithmodifier("A-S-X".to_owned()).unwrap(),
