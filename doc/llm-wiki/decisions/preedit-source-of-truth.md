...
IME preedit の描画位置は `phisical_layouter` が返す physical layout、特に `preedit_chars` を唯一の正とする。
...
- `text_buffer` の公開互換面と `ChangeEvent` 順序依存を崩さず、layout 計算責務だけを `phisical_layouter` に分離することが修正安全性に直結する

## 運用上の含意

- preedit 表示変更時は UI だけでなく `phisical_layouter` の layout 計算を先に確認する
- 関連修正では `editor.rs` の編集状態と `phisical_layouter` の計算結果の整合を確認する