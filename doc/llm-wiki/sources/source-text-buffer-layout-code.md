---
title: Source Summary - Phisical Layouter Code
kind: source
status: production
updated: 2026-07-19
source_refs:
...
# Source Summary - Phisical Layouter Code
...
- `calc_phisical_layout` は `Editor` から `buffer_chars/main_caret/mark_caret` を取得し、編集状態とレイアウト責務を分離する
- `PhysicalLayoutCalculator` は行スナップショットを走査し、折り返し、禁則、caret/mark、preedit を 1 本の経路で計算する
