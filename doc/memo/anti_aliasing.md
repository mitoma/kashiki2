...
終点L          : 5  (直線の終点)
制御点         : 6  (ベジェ曲線の制御点)
ベジェ補助直線始点 : 7  (ベジェ補助直線（塗りつぶし）専用の始点)
ベジェ補助直線終点 : 8  (ベジェ補助直線（塗りつぶし）専用の終点)
```

ベジェ曲線・ベジェ補助直線・直線の三種の三角形を頂点共用したままにすると、
共用頂点の `triangle_type` が混在し（例: 始点B/終点B が `(1,1,0)`）シェーダー側で区別できない。
そのため、ベジェ補助直線（塗りつぶし）三角形には専用頂点 (7, 8) を割り当てて頂点を共用しない。
これにより各三角形は均一な `triangle_type` を持つ。

### 三角形の構成

**ベジェ曲線部 (曲線本体 - アンチエイリアシング対象, triangle_type = (1,0,0))**
- 直前の終点B + 制御点 (6) + 終点B (4 または 2)

**ベジェ補助直線 (塗りつぶし用 - アンチエイリアシング対象外, triangle_type = (0,1,0))**
- 原点B (0) + ベジェ補助直線始点 (7) + ベジェ補助直線終点 (8)

**直線部 (アンチエイリアシング対象, triangle_type = (0,0,1))**
- 原点L (1) + 始点L (3) + 終点L (5)
